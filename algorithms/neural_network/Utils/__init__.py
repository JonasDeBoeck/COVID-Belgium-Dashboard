...
def run_region(region, sl1, sl, dt, pop, step=14, mAvg=False,
                is_SIR=False, model=Mod.SIR, is_semanal=False):
    """
        region: str
...
def filter_results(region, dia_ini, dia_fim, prev, pasta, inner_dir, coef_I = 1, coef_D = 1, coef_R = 1, 
    ...
    if recalc_rt:
        df_mean['Rt'] = df_mean['beta(t)']/(df_mean['Gamma']) * df_mean['Susceptible']/df_mean[['Susceptible','Infected','Recovered']].sum(axis=1)
        df_min['Rt'] = df_min['beta(t)']/(df_min['Gamma']) * df_min['Susceptible']/df_min[['Susceptible','Infected','Recovered']].sum(axis=1)
        df_max['Rt'] = df_max['beta(t)']/(df_max['Gamma']) * df_max['Susceptible']/df_max[['Susceptible','Infected','Recovered']].sum(axis=1)
...
def MAPE_DF(df_data, df_p, r, cols_p = ['Infected', 'Recovered'], cols_d=["At", 'Rt', 'Data'],prev = True): 
    # dc = pd.read_csv(f'data/dados - Agrupamento.csv')
    df_d = get_data(df_data,r,is_pred = False)
...
def run_unifica(dtime,case, prev=0,regs = None, unify = True, crop = 10, MM = False,
    ...
    if case == 'state':
        ...
        df_data = pd.read_csv('data/filtered_data/CASES_RECOVERED_DEATHS_ACTIVE.csv')
